<?php
require 'function.php'; // dari file yang kamu kirim (meng-handle POST add/edit/delete)
require 'cek.php';
...
if (isset($_GET['export']) && $_GET['export'] === 'excel') {
    ...
    if ($dari !== '' && $sampai !== '') {
        ...
        $where = "WHERE bm.tanggal BETWEEN '$d' AND '$s'";
    }
...
// ----------------- AJAX: getBarang (stok/kategori/ROP) -----------------
if (isset($_GET['getBarang']) && isset($_GET['id'])) {
    $id = (int)$_GET['id'];
    $res = mysqli_query($conn, "SELECT stok, COALESCE(kategori,'') AS kategori, stok_minimum, rata_pakai_harian, lead_time FROM barang WHERE id_barang = $id LIMIT 1");
...
<title>Barang Masuk - Final</title>
  ...
  <div id="layoutSidenav_nav">
    <nav class="sb-sidenav accordion sb-sidenav-dark" id="sidenavAccordion">
      <div class="sb-sidenav-menu"><div class="nav">
        ...
        <a class="nav-link active" href="barangmasuk.php">Barang Masuk</a>
    ...
    <main class="container-fluid px-4 mt-3">
      ...
      <div class="d-flex gap-2 align-items-center mb-3">
        <form method="GET" class="d-flex gap-2 align-items-end">
          ...
          <div class="align-self-end"><button class="btn btn-warning btn-sm">Filter</button> <a href="barangmasuk.php" class="btn btn-secondary btn-sm">Reset</a></div>
        </form>

        <div class="ms-auto d-flex gap-2">
          <a href="barangmasuk.php?export=excel<?= $dari ? '&dari='.urlencode($dari) : '' ?><?= $sampai ? '&sampai='.urlencode($sampai) : '' ?>" class="btn btn-success btn-sm"><i class="fas fa-file-excel"></i> Excel</a>
          <a href="barangmasuk.php?export=print<?= $dari ? '&dari='.urlencode($dari) : '' ?><?= $sampai ? '&sampai='.urlencode($sampai) : '' ?>" target="_blank" class="btn btn-danger btn-sm"><i class="fas fa-file-pdf"></i> PDF</a>
      ...
      <div class="card mb-4">
        <div class="card-body">
          <table id="tblMasuk" class="table table-striped table-bordered">
...
<div class="modal fade" id="singleEditModal" tabindex="-1">
  <div class="modal-dialog modal-lg">
    <form method="POST" class="modal-content" id="formEditMasuk">
      <input type="hidden" name="id_masuk" id="edit_id_masuk" value="">
      ...
      <div class="modal-body row g-3">
        <div class="col-md-6">
          ...
          <select name="id_barang" id="edit_id_barang" class="form-select" required>
            ...
            ?>
            <option value="<?=$b['id_barang']?>" data-kategori="<?=$kat?>" data-stok="<?= (int)$b['stok'] ?>" data-rata="<?= (int)($b['rata_pakai_harian'] ?? 0) ?>" data-lead="<?= (int)($b['lead_time'] ?? 0) ?>">
        ...
        <div class="col-md-4">
          <label class="form-label">ROP Sekarang</label>
          <input type="text" id="edit_rop" class="form-control" readonly>
        </div>
      ...
      <div class="modal-footer">
        <button type="submit" name="editbarangmasuk" class="btn btn-success">Simpan Perubahan</button>
      </div>
    </form>
  </div>
</div>
...
<div class="modal fade" id="singleDeleteModal" tabindex="-1">
  <div class="modal-dialog">
    <form method="POST" class="modal-content">
      <input type="hidden" name="id_masuk" id="del_id_masuk" value="">
      <div class="modal-header bg-danger text-white">
        <h5 class="modal-title">Hapus Data</h5>
        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
      </div>
      <div class="modal-body">Hapus data barang masuk <b id="del_nama_barang"></b> ?</div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary btn-sm" data-bs-dismiss="modal">Batal</button>
        <button type="submit" name="deletebarangmasuk" class="btn btn-danger">Hapus</button>
      </div>
    </form>
  </div>
</div>
  ...
  <div class="modal-dialog">
    <form method="POST" class="modal-content">
      ...
      <div class="modal-body">
        ...
        <label class="form-label small mt-2">Stok Awal</label>
        ...
        <label class="form-label small mt-2">ROP Sekarang</label>
        <input id="add_rop" type="text" class="form-control form-control-sm" readonly>

        <label class="form-label small mt-2">Tanggal</label>
        <input type="date" name="tanggal" class="form-control form-control-sm" required>

        <label class="form-label small mt-2">Jumlah</label>
      ...
      <div class="modal-footer">
        <button type="submit" name="addbarangmasuk" class="btn btn-primary btn-sm">Tambah</button>
      </div>
    </form>
  </div>
</div>
...
<script>
// init datatable
const dt = new simpleDatatables.DataTable("#tblMasuk", {
    perPage: 10,
...
document.addEventListener('click', function(e){
    ...
    if(editBtn){
        ...
        // set stok & rop from option if present
        ...
        if(opt){
            ...
            document.getElementById('edit_rop').value = (parseInt(opt.dataset.rata)||0)*(parseInt(opt.dataset.lead)||0);
        } else {
            // optionally fetch fresh via AJAX
            fetch('barangmasuk.php?getBarang=1&id='+id_barang).then(r=>r.json()).then(data=>{
                document.getElementById('edit_stokawal').value = data.stok ?? '';
                document.getElementById('edit_rop').value = (parseInt(data.rata_pakai_harian)||0)*(parseInt(data.lead_time)||0);
    ...
    if(delBtn){
        ...
        document.getElementById('del_id_masuk').value = id;
...
// when edit modal's select changes, update kategori/stok/rop
...
if(addSel){
    addSel.addEventListener('change', function(){
        ...
        document.getElementById('add_rop').value = (parseInt(opt.dataset.rata)||0)*(parseInt(opt.dataset.lead)||0);

        // fetch fresh server data optionally
        const id = this.value;
        if(id){
            fetch('barangmasuk.php?getBarang=1&id=' + id)
            .then(r=>r.json()).then(data=>{
                ...
                document.getElementById('add_rop').value = (parseInt(data.rata_pakai_harian)||0)*(parseInt(data.lead_time)||0);
            }).catch(()=>{});
        }
    });
}
